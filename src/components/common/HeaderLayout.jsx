      ...
      {!["/login", "/signup"].includes(path) && (
        ...
        >
          <div className="flex justify-between items-center w-full max-w-[1920px] h-[60px] px-[20px] sm:h-[70px] sm:px-[40px] md:h-[80px] md:px-[220px]">
            <button onClick={handleMobileModalOpen} className="relative w-[22px] h-[22px] cursor-pointer sm:hidden">
            ...
            >
              <Image src={img_logo} alt="최애의 포토" fill className="object-cover" />
            </Link>
            {/* 지수님 TODO: 인증/인가 하실 때 true 부분 변경하시면 됩니다. */}
            {true ? (
              <div className="flex justify-center items-center gap-[30px]">
                <p className="font-bold text-[14px]/[17px] text-gray-200 hidden sm:block">1,540 P</p>
                <div>
                  <Notification />
                </div>
                <div className="relative hidden sm:block">
                  ...
                  {isModalVisible && (
                    <TabletAndDesktopProfileModal handleTabletAndDesktopModalClose={handleTabletAndDesktopModalClose} />
                  )}
                </div>
                <div className="border-l-[1.5px] border-gray-400 h-[17px] hidden sm:block"></div>
                <Link href="/" className="font-notoSans font-normal text-[14px]/[17px] text-gray-400 hidden sm:block">
                  로그아웃
                </Link>
              </div>
            ) : (
              <div className="flex justify-center items-center gap-[30px]">
                <Link href="/login" className="font-medium text-[14px]/[17px] text-gray-200">
                  로그인
                </Link>
                <Link href="/signup" className="font-medium text-[14px]/[17px] text-gray-200 hidden sm:block">
                  회원가입
                </Link>
              </div>
            )}
